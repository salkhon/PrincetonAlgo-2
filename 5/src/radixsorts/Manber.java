...
public class Manber {
    ...
    private int[] index;         // offset of ith string in order, order -> suffix
    private int[] rank;          // rank of ith string, suffix -> num of lesser suffixes
    ...
    public Manber(String s) {
        ...
        text = s;
        index   = new int[n+1];
        rank    = new int[n+1]; // not the same as cumulative, because size is not R. Rank of actual suffixes
        newrank = new int[n+1];
    ...
    private void doit() {
        for (offset = 1; offset < n; offset += offset) {
            int count = 0;
            for (int i = 1; i <= n; i++) {
                if (rank[index[i]] == rank[index[i-1]]) count++; // counting equal offset keys
                else if (count > 0) {
                    // sort
                    int left = i-1-count;
                    int right = i-1;
                    quicksort(left, right); // fully sort on offset key, all the way
                    ...
                    for (int j = left + 1; j <= right; j++) {
                        ...
                        } // else can only be equal, because quicksort sorts all the way
                        newrank[index[j]] = r;
                    }
    ...
    private void msd() {
        ...
        // sort by first char
        for (int i = 0; i < n; i++)
            index[cumm[text.charAt(i)]++] = i; // order of suffix array is the same as the text only initially, so for only one pass msd
    }
